...
if [ -d "${tmpdir}" ] ; then
  cp -R /home/pring/13v/freeciv-web/freeciv/freeciv/build/../data/ruledit/ "${tmpdir}" || exit 1
  cp -R /home/pring/13v/freeciv-web/freeciv/freeciv/build/../data/default/ "${tmpdir}" || exit 1
else
...
for ruleset in $rulesets; do
  ...
  /home/pring/13v/freeciv-web/freeciv/freeciv/build/fcruleup -F -c -r $ruleset \
      --output "$resaved_path" \
    || exit 1

  echo "Trying to load $ruleset saved as $resaved"
  FREECIV_DATA_PATH=$tmpdir \
      /home/pring/13v/freeciv-web/freeciv/freeciv/build/tests/rs_test_res/ruleset_loads.sh "$resaved" \
    || exit 1

  echo "Checking ruleset saving stability for $ruleset"

  echo "Resaving resaved $ruleset"
  FREECIV_DATA_PATH=$tmpdir /home/pring/13v/freeciv-web/freeciv/freeciv/build/fcruleup -F -c -r "$resaved" \
